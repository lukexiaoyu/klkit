...
// .svelte-kit/output/server/entries/pages/one/_page.svelte.js
...
function validator(expected) {
  ...
  function validate(module, file5) {
    if (!module)
      return;
    for (const key2 in module) {
      if (key2[0] === "_" || set.has(key2))
        continue;
      const hint = hint_for_supported_files(key2, file5?.slice(file5.lastIndexOf("."))) ?? `valid exports are ${expected.join(", ")}, or anything with a '_' prefix`;
